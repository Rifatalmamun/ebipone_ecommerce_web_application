...
@php
    ...
    $shop_new_order = DB::table('order_childs')
                    ...
                    ->where('order_childs.status','0')->orderBy('id','DESC')->get();
...
$pickup_point_received_order = DB::table('order_childs')
                    ...
                    ->where('order_childs.isShopReceived','1')
                    ->where('order_childs.isShopSend','1')
                    ->where('order_childs.isWarehouseReceived','1')->orderBy('id','DESC')->get();
    ...
    <div class="site__body">
        ...
        <div class="block">
            <div class="container container--max--xl">
                                                ...
                                                <!-- SEARCH PART START -->
                        {{-- <form action="{{route('search_order_by_shop_and_pickup_point')}}" method="POST">
                            @csrf
                                <input type="hidden" name="page" value="new">

                            <div class="row mt-3 m-3 p-3" style="border: 1px solid rgb(224, 218, 218);">
                                    
                                        <div class="col-md-4"> 
                                            <div class="form-group"> 
                                                <select name="phone" class="form-control form-control-sm form-control-select2" required>
                                                    <option value="">Phone Number</option>
                                                    
                                                        @foreach ($search_data as $item)
                                                            
                                                        <option value="{{$item->b_phone}}"> {{$item->b_phone}} </option>
                                        ...
                                        <div class="col-md-4   ">
                                            <div class="form-group">
                                                <select name="transaction_id" class="form-control form-control-sm form-control-select2">
                                        ...
                                        <input type="submit" class="btn btn-primary btn-xs" value="Search" style="height: 30px;">
                ...
                <div class="row my-5">        
                    <div class="col">
                        ...
                        <table class="text-secondary" style="text-align: center">
                            <thead>
                                <tr>
                                    <th style="width: 5%; font-size:  12px; ">SL</th>
                                    <th style="width: 15%; font-size: 12px;">Order Date</th>
            
                                    <th style="width: 15%; font-size: 12px;">Product</th>
                                    <th style="width: 15%; font-size: 12px;">Size & Color</th>

                                    <th style="width: 5%; font-size:  12px;">Qty</th>
                                    <th style="width: 15%; font-size: 12px;">Ship.Address</th>
                                    <th style="width: 5%; font-size:  12px;">Invoice</th>
                                    <th style="width: 30%; font-size: 12px;">Select Pickup Point</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="font-size: 12px;">{{$item->order_date}} <br> {{$item->order_time}}</td>
                                    <td style="font-size: 12px;">
                                        <a href="{{URL::to($item->un_id.'/'.Str::slug($item->product_name))}}" class="text-secondary" target="_blank" style="text-decoration: none;">{{$item->product_name}}</a>
                                    </td>
                                    
                                    <td style="font-size: 12px;">{{$item->sz}}-{{$item->clr}}</td>
                    
                                    <td style="font-size: 12px;">
                                        @if ($item->qty<10)
                                            
                                            <span class="font-weight-bold text-danger">0{{$item->qty}}</span>

                                        @else
                                            <span class="font-weight-bold text-danger"> {{$item->qty}} </span>
                                        @endif

                                    </td>
                                    <td style="font-size: 12px;">{{$item->s_district}},{{$item->s_area}}</td>

                                        <td>
                                            
                                            <a href="{{URL::to('order-invoice-'.$item->id)}}" target="_blank"> <i class="fas fa-scroll text-danger" data-toggle="tooltip" data-placement="top" title="View Order Details"></i> </a>
                                        </td>
                                        ...
                                        <form action="{{route('send-to-warehouse')}}" method="POST">
                                            ...
                                            <input type="hidden" name="id" value="{{$item->id}}">
                                                ...
                                                @if (Auth::user()->id == $item->shop_id  )
                                                    @if ($item->isShopSend=='0')
                                                                ...
                                                                <td>
                                                                    <select class="form-control" name="warehouse_id" id="warehouse" required>
                                                                        ...
                                                                        @foreach ($pickupPoint as $item)
                                                                            ...
                                                                            <option value="{{$item->id}}">{{$item->w_name}} / {{$item->w_district}}</option> 
                                                    ...
                                                    @else 
                                                        @php
                                                            $warehouse = DB::table('warehouses')->where('id',$item->warehouse_id)->first();
                                                        @endphp
                                                        <td>
                                                            {{$warehouse->w_name}}
                                                        </td> 
                    ...
                    <div class="col-12 col-lg-12 mt-1 mt-lg-0">
                        <div class="dashboard">
                              ...
                              <div class="w3-bar mt-3" style="background: #F0F0F0">
                                ...
                                <button style="width: 25%; background: #CCCCCC;" id="ln" class="w3-bar-item w3-button" onclick="openCity('London')">New Order ({{$shop_new_order_count}})</button>
                                ...
                                <button style="width: 25%;" id="tk" class="w3-bar-item w3-button" onclick="openCity('Tokyo')">Received By Pickup Point ({{$pickup_point_received_order_count}})</button>
