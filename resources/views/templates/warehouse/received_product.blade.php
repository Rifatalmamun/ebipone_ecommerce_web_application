...
$auth_warehouse = DB::table('warehouses')->where('user_id',Auth::user()->id)->first();
    ...
    $upcomming_product = DB::table('order_childs')
            ...
            ->join('orders','orders.id','order_childs.order_id')
            ->select('order_childs.*','orders.status','orders.reason','orders.order_date','orders.order_time','orders.s_district','orders.shipping_address','users.phone','users.shop_phone','products.un_id','products.product_name','products.product_code','products.image_one')
            ->where('orders.status','complete')
            ->where('orders.reason','order_product')
            ->where('order_childs.warehouse_id',$auth_warehouse->id)
            ->where('order_childs.isShopReceived','1')
                ...
                <div class="col-12 col-lg-12 mt-1 mt-lg-0">
                    <div class="dashboard">
                          ...
                          <div class="w3-bar mt-3" style="background: #F0F0F0">
                            ...
                            <button style="width: 20%;" id="ln" class="w3-bar-item w3-button" onclick="openCity('London')">Comming Product ({{$count_upcomming_product}})</button>
                            <button style="width: 20%;background: #CCCCCC;" id="pr" class="w3-bar-item w3-button" onclick="openCity('Paris')">Received Product ({{$count_received_product}})</button>
                          ...
                          <div id="Paris" class="w3-container city" style="padding: 0 0; width: 100%;">
                                            ...
                                            <table id="received_order_table_id"  style="width:100%;">
                                                <thead style="background: #ebf1f5; color: rgb(0, 0, 0);">
                                                    <tr>
                                                        ...
                                                        <th style="width: 10%; font-size:  11px;">Print</th>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{URL::to('pickup-point-pirnt-invoice-'.$item->id)}}" style="display:block; text-decoration: none;" target="_blank" data-toggle="tooltip" data-placement="top" title="Print Invoice">Print</a>
                                                    </td>
